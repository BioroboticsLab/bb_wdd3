...
class R2Plus1D_YOLO_MultiHead(nn.Module):
    ...
    - Direction Head: dir_x, dir_y (waggle direction) - NOW WITH DILATED CONVS
    - Temporal Head: t_start, t_end (with richer temporal features via 3D conv stack)
    
    This allows each task to optimize independently with
    appropriate feature representations so that they do not compete for 
    capacity.
    
    Improvements:
    - Deeper 3D conv stack for temporal head (better temporal modeling)
    - Dilated convolutions in direction head (larger receptive field for movement patterns)
    """
    ...
    def __init__(
        ...
        # Shared feature projection (256 to 256) - KEEP FULL CAPACITY
        ...
        # Spatial confidence head - 3 layers: 256 to 128 to 64 to output
        ...
        # Direction head - NOW WITH DILATED CONVOLUTIONS
        # Dilated convs provide larger receptive field to see full waggle movement pattern
        # Predicts: [dir_x, dir_y] per detection
        self.direction_head = nn.Sequential(
            # Dilated conv: sees 5x5 area with 3x3 kernel (dilation=2)
            nn.Conv2d(256, 128, kernel_size=3, padding=2, dilation=2, bias=False),
            nn.GroupNorm(8, 128),
            nn.ReLU(inplace=True),
        ...
        # Temporal head with DEEPER 3D conv stack
        # Processes temporal features with multiple layers before pooling
        # This allows the network to better model temporal dynamics
        ...
        # Temporal prediction head (now takes 64 channels instead of 256)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Now uses dilated convolutions for larger receptive field
        direction_output = self.direction_head(shared_feat)
        
        # Temporal 
        # Process temporal features with 3D conv stack before pooling
