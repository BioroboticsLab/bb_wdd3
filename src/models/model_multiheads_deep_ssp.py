...
class SPP(nn.Module):
    ...
    Aggregates features at multiple scales to capture context at different receptive fields.
    """
    ...
    def forward(self, x):
        ...
        Args:
            x: (B, C, H, W)
        Returns:
            Pooled features (B, out_channels, H, W)
        """
        pooled_features = [x] + [pool(x) for pool in self.pools]
...
class R2Plus1D_YOLO_MultiHead(nn.Module):
    ...
    Added Spatial Pyramid Pooling (SPP) for multi-scale context aggregation.
    """
    ...
    def __init__(
        ...
        # Spatial Pyramid Pooling - captures multi-scale context
        self.spp = SPP(in_channels=256, out_channels=256, pool_sizes=[5, 9, 13])
        
        # Temporal aggregation for spatial features
        self.temporal_pool = Soft_Temporal_Pool(channels=256, reduction=4)
        
        # Shared feature projection (256 -> 256) - KEEP FULL CAPACITY
        ...
        # Spatial confidence head - 3 layers: 256 -> 128 -> 64 -> output
        ...
        # Direction head - 3 layers: 256 -> 128 -> 64 -> output
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Apply SPP for multi-scale context aggregation
        pooled_feat = self.spp(pooled_feat)
